...
public class NewtonsoftJsonOptionsConfigurationServiceCollectionExtensionsTests
{
    [Theory]
    [AutoData]
    public async Task ConfigureAsJson_can_bind_options(TestClass testClass)
    {
        ...
        var hostBuilder = Host.CreateDefaultBuilder()
            ...
            {
                services.ConfigureAsJson<TestClass>(context.Configuration);
            });
    ...
    [Theory]
    [AutoData]
    public async Task ConfigureAsJson_can_bind_options_with_name(TestClass testClass, string name)
    {
        testClass.NullValue = null;
        testClass.NullList = null;
        var hostBuilder = Host.CreateDefaultBuilder()
            ...
            .ConfigureServices((context, services) =>
            {
                services.ConfigureAsJson<TestClass>(name, context.Configuration);
            });
        using (var host = hostBuilder.Build())
        {
            await host.StartAsync();
